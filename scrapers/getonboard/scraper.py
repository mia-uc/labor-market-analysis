...
class GetOnBoardScraper(HttpScraper):
    ...
    def logger(self, index, job, already):
        print(
            f"From {job['published_at']} Job {'✅' if already else '🆕'} #{index} ==> {job['title']}")
    ...
    def __get_job__(self, job: dict) -> list[dict]:
        ...
        print(
            f"🪲 GetOnBoard Scraper are Finding Details for {job['id']} - {job['title']}")
        ...
        if (tags := soup.find('div', {"itemprop": "skills"})):
            new_info["jobs_tags"] = [a.get_text().strip()
